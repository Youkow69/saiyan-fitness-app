...
function checkForPR(
  ...
  // Also check active workout sets already logged
...
function OnboardingView({
  ...
  const [answers, setAnswers] =
    useState<OnboardingAnswers>(defaultOnboardingAnswers)
...
// ── HOME VIEW ─────────────────────────────────────────────────────────────────
// (Kept exactly as Sprint 1, but now reads from context)
    ...
    >
      {items.map(({ id, icon, label, ariaLabel }) => (
        ...
        >
          ...
          />
          {/* Timer badge on the Training tab */}
...
function AppInner() {
  ...
  const addSet = useCallback(
    (
      ...
      // Check for PR before adding set
